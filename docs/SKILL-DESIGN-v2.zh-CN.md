...
在当时确认的目标边界尚未覆盖时，中断、沉默、换 task、切换主题、诊断未完成或单纯停止都不
保存。用户明确缩小目标后按新边界重新判断。v2 不承诺跨对话恢复，也不把未完成学习伪装成
“学过”。
...
4. 确定学习目的、目标深度和起点；广泛学习必须形成明确的起点判断，目标较浅只缩短完成边界，
   不取消诊断；
5. 形成 1–5 个当前对话内稳定的完成项；轻量“了解一下”通常只需 1–3 项，不能扩成开放课程；
6. 用自然语言告诉用户选择的起点和本次承诺边界，不展示内部 ID，但让用户能判断何时结束；
7. 立即开始第一段实质讲解。

起点诊断沿用 v1 的低摩擦硬约束：

- 当前请求已明确给出相关前置时直接采用，不重复问；
- 当前对话或 Vault 有近期、直接且相关的可靠证据时，用它完成诊断；只有一个关键前置陈旧、
  矛盾或不确定时，问至多一道窄确认。knowledge 页、旧 learning record、一次答对或模型自己的
  摘要都不能单独证明长期掌握；
- 否则一次给出恰好三道简短知识选择，只测会改变第一段讲法的前置。每题都有“看到选项前不
  知道/主要靠猜”；不够简单的题另有“看不懂这些选项在说什么”，两者不能合并；
- 用户拒绝时把关键前置视为未知，从保守起点立即开讲，不再劝答；只有答案冲突到无法选择安全
  起点时才追加一道问题并说明原因。

目标深度与三道知识诊断分开。用户已给出可映射的结果或清楚限制，例如“只想大概了解、不用
太深”，采用满足其目的的最低边界并明确说明，不重复问深度；否则在同一轮另给一道面向结果的
深度选择，它不计入三题，也不能从诊断表现推断。候选结果仍为建立地图、讲清完整机制、带提示
处理典型案例、较少提示迁移到新案例，以及“不确定，请推荐”。
...
“讲解已交付”与“长期结果已保存”严格分开。每次准备新的教学单元前，Skill 必须先用此前已经
交付的 assistant 消息结算完成项和范围内困惑。仍有完成项时，只继续约定边界内的下一项；已经
全部覆盖时必须立即主动收束，不能继续推荐相邻章节，也不能等用户询问“是不是学完了”。
...
“到这里就够了”“我只想大概知道”等明确表达可以修改原目标边界。Skill 按用户新确认的较浅
边界重新判断：已交付内容足够时正常进入主动收束；只有“先停一下”“改天继续”等没有改变目标
的停止，才属于未完成且不保存。不能把所有停止措辞机械归为中断，也不能因用户困惑而擅自缩小
目标。
...
首次呈现时必须先说明它控制的是未来教学行为，不是能力评估或学习者标签；暂不设置只表示没有
额外的长期教学默认，不会关闭学习、知识检索或完成后保存。每个入口都说明结果：暂不设置不会
创建空 profile，快捷选择用少量可跳过问题形成默认，自由讲述只提取可执行的长期行为，组合入口
允许先定大方向再补条件和例外。不能只列选项名称让不了解内部设计的用户猜含义。
...
6. 新独立 Vault 明确区分三种 Git 结果：启用 Git 且以后自动提交（默认）、建立初始 baseline
   ...
   Obsidian 只是一项当次 UI 选择，不写进配置。首次呈现 Git 选择时先用结果名称，再把
   `managed/true`、`managed/false`、`off/false` 作为括号内的精确内部值；说明 baseline、后续
   自动 commit 与完全关闭分别意味着什么，并明确所有模式都不会自动创建远端、登录托管账号、
   添加 remote 或 push；
...
- 首次用户看到 Learning Guidance 和 Git 选项时，先理解它们控制什么、选择后的实际结果、
  不会发生什么以及能否重配；内部枚举不能充当唯一说明；
...
- 广泛学习必须有明确起点判断；没有近期、直接、相关证据时默认一次给出恰好三道前置知识题，
  浅目标不能成为跳过诊断的理由；
- “不知道/主要靠猜”和“看不懂选项”保持可区分，拒绝诊断后从保守起点开讲；
- 目标深度与知识诊断分开；“大概了解、不用太深”等清楚限制映射到小而可观察的边界，不额外
  追问深度，也不取消三题诊断；
- 诊断后用自然语言说明起点与承诺范围，用户无需看到内部 ID 也能知道何时完成；
- 每个新教学单元前结算此前已交付内容；边界覆盖后必须主动说明完成并提供结束保存、巩固或
  扩展选择，不能用泛化的“下一段”无限续课；
- 明确缩小目标后按新边界判断是否完成；单纯暂停或离开仍是不保存的未完成学习。
